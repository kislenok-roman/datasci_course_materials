...
splitDataset <- function() {
    all <- readRDS("temp/all-data-with-coords-v1.RData") # we want consistent train/test/validation datasets
    ...
    list(train = train, test = test, validation = validation, product = product)
}

if (!file.exists("temp/train.RData") || !file.exists("temp/test.RData") ||
    !file.exists("temp/validation.RData") || !file.exists("temp/product.RData")) {
    datasets <- splitDataset()

    saveRDS(datasets$train, "temp/train.RData")
    saveRDS(datasets$test, "temp/test.RData")
    saveRDS(datasets$validation, "temp/validation.RData")
    saveRDS(datasets$product, "temp/product.RData")
    rm(datasets)
} else {
    train <- readRDS("temp/train.RData")
    test <- readRDS("temp/test.RData")
    validation <- readRDS("temp/validation.RData")
    product <- readRDS("temp/product.RData")
}
...
if (FALSE) {
    ...
    # I don't know why we should ignore all 'Dismantle' events. There are no reasons I think.
    # We want our evaluation to have a proportion as in a real dataset.
